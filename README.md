...
- **APScheduler background jobs** — alert evaluation (every 10min), auto-buy engine (every 10min), commodity alerts (every 30min), BTC trailing-stop bot (every 15min), signal pruning (daily); discovery jobs (buy zones, theme scores, live scanner, idea generator) registered but default-paused to conserve Render Starter resources
...
- **BTC trailing stop bot CLI** (`btc-bot/`) — `btc_trailing_bot.py` executes a $-denominated BTC/USD paper trade via Alpaca with three rules: hard floor (−10% stop loss), trailing floor (activates at +10%, steps every +5%, never moves down), and a 3-level DCA ladder re-entry (−20% → $10k, −30% → $15k, −40% → $20k — larger buys at deeper discounts; $10k default initial buy, max $55k deployed); companion scripts `btc_execute_now.py` (one-shot buy), `btc_close_now.py` (one-shot close), and `btc_status.py` (read-only check) for manual session control. See `btc-bot/BTC_BOT.md` for the full spec.
...
- **PIN Auth (V8)** — 4-digit PIN for quick repeat login after the first magic-link sign-in; bcrypt-hashed PIN stored per user with 5-attempt lockout and 15-min cooldown via `user_pins` table; auth flow: PIN verify → server generates Supabase admin one-time link → client exchanges via `verifyOtp` → session restored
- **BTC Trailing-Stop Bot — Web Feature (V9)** — `/btc-bot` page exposes the BTC strategy as a multi-tenant, DB-backed, APScheduler-driven feature. Pure decision service (`evaluate_tick(...) -> TickAction`) with zero I/O drives all rule logic; orchestrator iterates active+cooldown sessions across users every 15 min; auto re-enters after a 4 h cooldown post stop-out; adopts an existing Alpaca position when no DB session exists; manageable from `/crons` like every other cron. Two tables (`btc_bot_sessions` + sparse `btc_bot_actions` audit log with partial-unique constraint preventing two live sessions per user). 57 unit tests cover every decision branch + orchestrator dispatch + API endpoint. Replaces the earlier `monitor_btc_bot` read-only heartbeat.
...
- **BTC Bot (V9)** — active session card with original/blended entry, current floor (up-only badge), trailing state, ladders fired; cooldown panel with re-entry countdown; cross-session action history (filterable by action type) with floor-delta deltas and Alpaca order IDs
...
| BTC trailing stop bot | `btc-bot/` — paper trade BTC/USD via Alpaca; hard floor (−10%), trailing floor (activates +10%, steps every +5%), 3-level DCA ladder re-entry (−20%/$10k, −30%/$15k, −40%/$20k); $10k default initial buy ($55k max deployed); companion scripts `btc_execute_now.py`, `btc_close_now.py`, `btc_status.py` for manual entry/exit/inspection |
...
| PIN Auth (V8) | 4-digit PIN for quick repeat login after first magic-link sign-in; bcrypt-hashed in `user_pins` table; 5-attempt lockout with 15-min cooldown; server uses Supabase admin `generate_link` + client `verifyOtp` to restore session |
| BTC Trailing-Stop Bot — Web (V9) | `/btc-bot` — full-lifecycle trader (initial buy → ladder → trailing floor → stop-out → 4h cooldown → auto re-enter) running as an APScheduler tick every 15 min; multi-tenant `btc_bot_sessions` + sparse `btc_bot_actions` audit log; pure `evaluate_tick(...) -> TickAction` decision service has zero I/O and 21 unit tests cover every branch; appears on `/crons` like every other job |
...
# Custom entry size
BTC_USD=5000 python ../btc-bot/btc_trailing_bot.py
```
