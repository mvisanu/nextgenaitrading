...
const MODE_BADGE_CLASSES: Record<StrategyMode, string> = {
  ...
  squeeze: "bg-primary/10 text-primary border-primary/30",
};
...
function BacktestPanel({
  ...
  const mutation = useMutation<RunResult, Error, RunStrategyRequest>({
    mutationFn: async (req) => {
      ...
      let run: import("@/types").StrategyRun;
      ...
      const trades = await backtestApi.trades(run.id);
      const summary = summarizeBacktest(run, trades);
      const { total_return_pct: totalReturnPct, max_drawdown_pct: maxDrawdownPct, sharpe_like: sharpeLike } = summary;
  ...
  return (
    <div className="mt-1 rounded-sm border border-border/10 bg-surface-lowest">
      <div className="px-4 py-3 border-b border-border/10">
        <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Test reference mode</p>
...
export default function StrategySamplesPage() {
  ...
  return (
    <AppShell title="Strategy Samples">
      <p className="mb-4 text-sm text-muted-foreground">Technique examples are learning templates. Reference tests run the predefined app mode, not the example Pine Script. Review and test the exact script in TradingView before using its rules.</p>
      {/* ── Page header ── */}
      <div className="mb-6">
                ...
                <div className="flex flex-col h-full bg-surface-low border border-border/10 rounded-sm">
                  ...
                  <div className="flex-1 px-4 pb-4 space-y-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        <Play className="h-3 w-3" />
                        {isBacktestOpen ? "Hide Run" : "Test reference mode"}
                      </Button>
                    </div>
                  </div>
                </div>

                {/* ── Test reference mode panel ── */}
                {isBacktestOpen && (
                  <BacktestPanel
                    sample={sample}
