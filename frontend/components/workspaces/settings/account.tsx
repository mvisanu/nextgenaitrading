"use client";
import { BrowserDataRecovery } from "@/components/layout/BrowserDataRecovery";
...
const profileSchema = z.object({
  ...
  default_mode: z
    .enum(["conservative", "aggressive", "ai-pick", "buy-low-sell-high", "squeeze"])
    .optional(),
});
...
export default function ProfilePage() {
  ...
  const { mutate: createCredential, isPending: creatingCred } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: tradingKeys.credentials(user?.id) });
                            ...
                            >
                              ...
                              <td className="px-6 py-4 text-right">
                                <div className="flex items-center justify-end gap-2">
                                  ...
                                  <button
                                    aria-label={`Delete ${cred.profile_name}`}
    ...
    <div className="bg-surface-mid border-l border-primary/40 p-4 rounded-sm relative group">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          <button
            aria-label={`Delete ${credential.profile_name}`}
            onClick={onDelete}
            className="text-muted-foreground hover:text-destructive transition-colors"
          >
...
function SecurityPanel() {
  return (
    <>
      <h2 className="mb-5 text-sm font-semibold">Password & alerts</h2>
      <PasswordSection />
      <a className="mt-6 inline-flex min-h-11 items-center text-sm text-primary underline" href="/research?view=alerts">Manage price alerts</a>
    </>
  );
}
